...
import model.{MaxMinHolder, MaxMinHolderBD, MeanHolder, MeanHolderBD}
import org.apache.spark.SparkContext
import org.apache.spark.rdd.RDD
import org.apache.spark.sql.Row
import utils.{CSVParser, CalendarManager, ProfilingTime, Statistics}

object Query3BD extends Serializable {
  ...
  def executeCSV(sc: SparkContext, data: RDD[String], cm: CalendarManager)
    ...
    val q = data
      ...
      .reduceByKey(
        (x,y) => Statistics.computeOnlineMaxMinBD(x,y)
      )
      .map(
        d =>  {
          val house = d._1._1
          ...
          ((house, household, plug, rate), new MeanHolderBD(d._2.range(), 1))
        }
      )
      .sortByKey() // TODO check perché togliendo questa cambia il risultato
      .reduceByKey(
        (x,y) => Statistics.computeOnlineMeanBD(x,y) // average on month per rate
      ).map {
        case (k, v) =>
