...
docker run -t -i -p $PORT:$PORT -e NIFI_WEB_HTTP_PORT="$PORT" -e HDFS_DEFAULTS_FS="$HDFS_DEFAULT_FS" --network=hadoop_network -d --hostname=$HOSTNAME --name=nifi zanna94/nifi
